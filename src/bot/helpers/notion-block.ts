  ...
  entities.forEach(entity => {
    for (let i = entity.offset; i < entity.offset + entity.length; i++) {
      ...
      if (entity.type === 'bold') annotatedChar.annotations.bold = true
        ...
        const url = encodeURIComponent(text.slice(entity.offset, entity.offset + entity.length))
        annotatedChar.text.link = {url}
      } else if (entity.type === 'text_link') {
        const url = encodeURIComponent(entity.url)
        annotatedChar.text.link = {url}
...
export function convertFileToNotionBlock(file: File): BlockObjectRequest {
  if (file.type === 'image') {
    return {
      object: 'block',
      image: {type: 'external', external: {url: getFileUrl(file)}},
    }
  } else if (file.type === 'video') {
    return {
      object: 'block',
      video: {type: 'external', external: {url: getFileUrl(file)}},
    }
  } else if (file.type === 'audio') {
    return {
      object: 'block',
      audio: {type: 'external', external: {url: getFileUrl(file)}},
    }
  } else {
    if (file.extension === 'pdf') {
      return {
        object: 'block',
        pdf: {type: 'external', external: {url: getFileUrl(file)}},
      }
    }
    return {
      object: 'block',
      file: {type: 'external', external: {url: getFileUrl(file)}},
    }
  }
}
